...
| DB | Supabase 프로젝트 ref `suoaovdanimacisbocuk`, 마이그레이션 **0001~0006 전부 적용** |
...
### 3.1 ✅ DB 적용·정리 완료 (2026-09-09)

**사용자가 실행해야 하는 SQL 은 더 없다.**

| | |
|---|---|
| 0005 삭제 함수 | 적용. **회원이 자기 글을 삭제할 수 있다** |
| 0006 anon 권한 회수 | 적용. 비회원 호출이 `permission denied for function` 으로 막힘 (전에는 함수 안 검사가 막고 있었다 — §5.11) |
| 시드 조회수 | 적용. 26건이 실제 자릿수(최대 3,204) |
| 테스트 잔여 글 | 10건 전부 정리. 현재 글 27건 = 시드 26 + 공지 1 |
| RLS 통합 테스트 | **37/37 통과** (삭제 경로 3종 포함) |

### 3.2 다음에 할 일 — P1-D 남은 세 칸
...
| `ae938ae` | README — 기능 목록·버전 기록·페이즈 계획 |
| `2c1ff29` | 눌러야 하는 주소를 코드블록에서 링크로 (클릭이 안 됐다) |
| `a9ae3da` | **0006** anon EXECUTE 실제 회수 + 검증 출력을 판정 문장으로 |
