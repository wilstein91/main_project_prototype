...
| 디자인 | **먹빛 바탕 + 금색 프레임 + 그림** (v1.3) — PRD §6.2 · DESIGN §4·§10 |
| 그림 | `public/images/` 에 11장. 없으면 SVG 폴백 (DESIGN §10.9). 다시 뽑으면 파일만 갈아 끼운다 |
| 서체 | 본문 Pretendard · **제목/메뉴/버튼 나눔명조** (DESIGN §5) |
| 접근성 | 대비·터치·키보드 **실측 통과** (DESIGN §8). 다크 전환 후 재실측 완료 |
| 콘텐츠 | 시드 글 26건 · 댓글 48개 · 시드 계정 7개 (오픈 전 삭제 — FINAL_CHECKLIST D-3) |

**Phase 1 v1.1 이 무엇인가**: 기능과 디자인 기반 작업(P1-D)이 **전부**
끝난 상태. Phase 1 에 남은 것은 **실기기 QA 한 칸**이고 코드 작업이 아니다.
...
### 3.2 ✅ P1-D 완료 (2026-09-09)

여백·문안·접근성 세 칸을 끝냈다. 판단 기준은 전부 `DESIGN.md` 에 적었다.

| | 무엇을 했나 | 어디 |
|---|---|---|
| 여백 | 4px 배수 하나로 통일. 카드가 없는 화면(글쓰기·내 정보)에도 `lg:px-6` 을 줘서 **글이 시작하는 x 좌표를 모든 화면에서 같게** 맞췄다 | DESIGN §7 |
| 문안 | 빈 상태는 `사실 + 다음 행동`, 오류는 `~해 주세요` 하나로. 네 파일에 복붙돼 있던 문구를 `result.ts` 로 모았다 | DESIGN §2.1·§2.2 |
| 접근성 | 대비·터치 44px·키보드 초점. **화면을 렌더해서 실측**했다 | DESIGN §8 |

고친 것 중 화면으로는 안 보이던 것 셋:

1. **보조 텍스트가 브랜드색 면 위에서 4.31:1** — 흰 바탕과 회색 면에서는
   통과하고 있었다. 홈 상단 띠와 `/about` 히어로가 그 배색이다.
   토큰(`--ink-sub`)을 한 칸 어둡게 해서 고쳤다.
2. **입력칸에 `outline-none`** — 키보드로 이동하면 지금 어디에 있는지
   알 수 없었다. `globals.css` 한 곳에서 초점 테두리를 정하도록 바꿨다.
3. **오류 메시지가 사용자가 할 수 없는 일을 안내** — 메일 한도 오류에
   "커스텀 SMTP 를 연결해 주세요" 가 그대로 나갔다. 운영자용 내용은
   서버 로그로 보내고 화면에서는 뺐다.

### 3.3 로드맵이 바뀌었다 — PRD v0.4 (2026-09-09)

Phase 1 과 무관하지만, **다음 페이즈를 열기 전에 읽어야 한다.**

| | |
|---|---|
| Phase 재편 | 3 = 정착 **+ 종목 축**, **4 = 시황 정보(뉴스·공시) 신설**, 5 = 확장 **+ 안정화**(끝에 몰아서). Phase 5 로 끝낸다 |
| 새로 들어온 것 | 회원 등급제 · 종목 페이지 `/s/[ticker]` · 전일 종가 · 화제 종목 순위 · 뉴스 공유 게시판 · 뉴스 자동 수집 · DART 공시 |
| **빠진 것** | **유료 구독·멤버십 전면 제외.** 이 서비스는 전 기능 무료다. 수익은 Phase 5 배너 광고뿐. `features.ts` 에서 `perkSubscription`·`paidResearch` 키를 지웠다 |
| 빠진 것 | FAQ·문의 폼 — 별도 자문사 웹사이트가 담당 |
| **빠진 것** | **자문사 트랙 전체.** `/company`·`/advisory`·리서치 게시판과 게이트 코드(`GatedLink`·`UnderConstruction`·`gate()`)를 제거하고 [ADVISORY_TRACK_MOVED.md](ADVISORY_TRACK_MOVED.md) 로 옮겼다 (O-9 종결). **이 저장소에는 이제 규제 게이트가 없다** |
| 불변조건 | **I-2·I-3 을 다시 썼고 I-5 를 신설했다.** 새 기능을 넣기 전에 PRD §3.2 를 다시 읽을 것 |

**시세는 무료로는 전일 종가까지다.** 장중 지연시세는 거래소 정보이용 계약
(사업자등록 전제)이 필요하다. 그래서 **어댑터 한 겹 뒤에 두고** MVP 는 전일
종가로 간다. 교체할 때 **데이터와 "종가 기준" 표기를 같은 커밋에서 함께**
바꿔야 한다 — 데이터만 바꾸면 허위 표기가 된다 (FINAL_CHECKLIST §F-1).

**뉴스는 본문을 저장하지 않는다.** 제목·200자 이내 인용·원문 링크·매체·시각만.
사진은 원문 미리보기(`og:image`) 1장. 규칙은 PRD §A-6 에 있고 게시판 상단
공지로 고정한다.

### 3.4 디자인 톤이 바뀌었다 — v1.2 (2026-09-13)

흰 바탕 + 청록 포인트 → **먹빛 바탕 + 금색 프레임**. 참고자료는 삼국지
게임의 무장 제안 화면이고, 근거는 PRD §6.2, 규칙은 DESIGN §10 이다.

핵심은 **새 색을 들여온 게 아니라는 것**이다. 로고를 그릴 때 정한
먹·청록·금·주홍 4색이 이미 토큰에 있었고, "UI 크롬에 쓰면 무거워진다" 는
이유로 로고 안에만 갇혀 있었다. 그 빗장을 풀었다.

**뒤집을 때 깨진 것 셋** (다음에 테마를 또 바꾸면 여기부터 본다):

1. **주요 버튼의 흰 글자** — 배경이 밝은 청록으로 바뀌었는데 글자는 흰색
   그대로라 **2.4:1** 로 떨어졌다. 글자를 바탕색(먹)으로 바꿔 7.9:1.
   밝기를 뒤집으면 **버튼 위 글자가 가장 먼저 깨진다**
2. **워드마크 SVG** — `BladeMark`·`BladeCrest` 는 `--color-ink` 를 stroke 로
   쓰므로 토큰만 바꾸면 따라왔지만, `wordmark.svg`·`lockup.svg` 는 **파일 안에
   색이 굳어 있어** 안 따라온다. DESIGN §3.4 가 "라이트로 확정했으므로 지금은
   문제되지 않는다" 고 적어 둔 바로 그 지점이다. 파일 2개 +
   `scripts/build_logo.py` 의 `COLORS` + `scripts/_arm.svgfrag` 를 함께 뒤집었다
3. **무기 그림의 자루** — 먹색 채움이라 먹빛 바탕에서 사라졌다. 윤곽선은
   밝게, 자루 몸통은 어두운 나무색으로 갈랐다

### 3.5 Phase 1 완료로 치기 전 남은 것

- **실기기 QA (iOS Safari · Android Chrome)** ← 이제 이게 다음 차례다.
  터치 44px 규칙을 `@media (pointer: coarse)` 로 걸었으므로 **실기기에서
  확인해야 한다** — 데스크톱 브라우저의 모바일 에뮬레이션으로는 통과했지만
  실제 기기에서 다시 본다
...
## 9. Phase 1 v1.1 에서 한 일 — P1-D 마무리

| | 내용 |
|---|---|
| 여백 | 카드 안쪽 좌우 20 → 24px, 목록 행 14 → 16px, 폼 필드 16 → 20px, 섹션 32px. 카드 없는 화면에 `lg:px-6` 을 줘 글 시작 x 좌표 정렬 |
| 문안 | 빈 상태 문장 통일, 검증 문구 `~하세요` → `~해 주세요`, 네 파일에 복붙된 문구를 `result.ts` 로, 사용자가 할 수 없는 일을 안내하던 오류 제거 |
| 접근성 | `--ink-sub` 한 칸 어둡게, `--field` 신설(입력칸 테두리 3.36:1), 초점 테두리 전역 규칙, 터치 44px 규칙, 오류를 `role="alert"` 로 |

바뀐 화면: 전부. 다만 **눈에 띄게 달라진 곳은 없다** — 여백이 맞고 문장이
같아진 것이라 개별 화면은 전과 비슷해 보이는 게 맞다. 화면을 옮겨 다니면
본문이 튀지 않는 것으로 확인한다.
