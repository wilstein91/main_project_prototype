...
| 실기기 | 갤럭시 / Android Chrome 통과 (2026-09-13) |

**Phase 1 (MVP) 은 끝났다.** 기능·디자인·접근성·테스트·실기기 QA 전부
닫혔다. **지금 차례는 Phase 2 이고, 첫 작업은 커스텀 SMTP(P2-F) 다.**

---

## 3. 먼저 할 일

### 3.1 P2-F 커스텀 SMTP — 여기부터

**왜 이것부터인가.** 지금은 비밀번호를 잊은 회원이 복구할 방법이 아예
없고, 오픈 전 반드시 켜야 하는 **이메일 인증**(FINAL_CHECKLIST D-1)도 같은
메일 발송에 묶여 있다. 하나로 둘이 풀린다.

Supabase 기본 메일은 **하루 발송 한도가 매우 낮아** 개발용 이상으로는
쓸 수 없다. 외부 메일 서비스(Resend 등)를 Supabase 의 SMTP 설정에 연결한다.

| 누가 | 무엇 |
|---|---|
| AI | 코드·설정·검증 |
| 사용자 | 메일 서비스 가입, 발신 도메인 확인, **API 키 한 개** 전달 |

> 안내 전에 Supabase 대시보드 메뉴 경로를 **실제로 확인한다.** 기억으로
> 쓰면 "그런 메뉴 없다" 는 답이 돌아온다 (§4.5).

### 3.2 그 다음 — Phase 2 나머지
...
**새 테이블을 만들면 RLS 정책과 `tests/rls/policies.test.ts` 케이스를
같이 넣는다.** 이것이 Phase 2 의 완료 조건이다.

### 3.2.1 Phase 1 에서 뒤로 넘긴 것 (잊지 말 것)

| 넘긴 것 | 어디로 |
|---|---|
| 비밀번호 재설정 메일 발송 | Phase 2 · P2-F ← 바로 위 |
| iPhone(Safari) QA · 햇빛 아래 가독성 | Phase 5 최종 점검 |
| 목록의 글 제목 서체 재검토 (상세는 명조, 목록은 고딕) | Phase 5 최종 점검 |
| `supabase gen types` 로 `src/types/database.ts` 자동 생성 | 아무 때나 |
...
**페이즈 완료와 오픈은 다른 관문이다.** [FINAL_CHECKLIST.md](FINAL_CHECKLIST.md)
를 전부 통과해야 실사용 오픈이며, 지금 비어 있는 칸이 최소 이만큼이다.

- **이메일 인증** 꺼져 있음 (D-1) — P2-F 와 함께 켠다
- **검색엔진 색인** 차단 중 (D-2)
- **시드 글 26건 · 시드 계정 7개** 삭제 (D-3)
- CSP `script-src` 미적용 (C-8) — Next 하이드레이션 nonce 배선 필요
- iPhone 실기기 QA (G)

---

## 보안검증 결과 (2026-09-09) — 취약점 4건 수정
