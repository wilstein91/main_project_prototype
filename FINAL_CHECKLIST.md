...
- [ ] 커뮤니티 트랙 불변조건 **I-1~I-5** 유지 (PRD §3.2)
      (운영자 투자의견 없음 / **이용자 요금 없음** / **운영 주체가 종목을 고르지 않음** /
      수익률 표방 없음 / **외부 자료에 해석 안 붙임**)
...
- [ ] `/advisory` · `/company` · `/research` 가 **404** — 자문사 트랙은 이 사이트에 없다 (PRD §3.4)
- [ ] `COMPANY.advisorySiteUrl` 이 `null` — **자문업 신고·등록 전에는 반드시 null.**
      켜면 커뮤니티가 미등록 자문 서비스를 광고하는 모양이 된다
- [ ] 푸터에 `자문업 등록·신고번호: 해당 없음 (미신고)` 표기가 **남아 있다**
      (자문사 트랙을 뺐다고 이 줄까지 지우면 안 된다 — PRD C-3)

### F-1. 시세·뉴스 표기 (Phase 3 이후 — 해당 기능 공개 시)

> Phase 1 오픈 시점에는 해당 없음. **종목 시세를 공개하는 날부터** 이 절이
> 살아난다.

- [ ] **시세 화면에 출처와 기준 시점이 있다** — "출처: 한국거래소 · YYYY-MM-DD **종가 기준**"
- [ ] 값만 있고 시점이 없는 시세 화면이 **하나도 없다** (PRD D-6 · §9)
- [ ] **⚠️ 장중 지연시세로 교체할 때 — 데이터와 문구를 같은 커밋에서 함께 바꾼다**
      - [ ] 어댑터 구현체 교체 (PRD §9 교체 용이성)
      - [ ] 표기를 `종가 기준` → `약 N분 지연` 으로 변경
      - [ ] **정보이용 계약 체결 확인** (PRD C-10 · O-8)
      - [ ] 계약이 허용하는 표시 범위·출처 표기 문구를 계약서대로 반영
      > 데이터만 바꾸고 문구를 그대로 두면 **허위 표기**가 된다.
      > 반대로 문구만 먼저 바꾸면 실제보다 최신인 것처럼 보인다.
- [ ] **종목 순위 화면에 집계 기준 + "매수·매도 권유 아님" 고지** (PRD §A-5-2)
- [ ] 화면에 쓰인 고지 문구의 지표가 **실제 집계 지표와 일치한다**
- [ ] **뉴스에 기사 본문이 저장돼 있지 않다** — 제목·200자 이내 인용·링크·매체·시각만 (PRD C-11)
- [ ] 모든 뉴스 항목에서 **원문으로 나갈 수 있다**
- [ ] 뉴스 사진이 원문 미리보기(`og:image`) 1장이며, 내려받아 재업로드한 것이 아니다
- [ ] 뉴스 게시판 상단에 **인용 규칙 공지**(PRD §A-6)와 **공유 고지**(§A-4)가 있다
- [ ] 뉴스 공유가 **지정 등급 이상에게만** 열려 있다 (PRD §5.1 · D-8)

---

## G. 실기기 QA ⚠️ AI 가 대신할 수 없다

> **1차 결과 (2026-09-13) — 갤럭시 / Android Chrome 통과.**
> 화면이 제대로 나오고, 터치·입력·스크롤에 이상이 없다는 것을 사용자가
> 실기기로 확인했다. 이것으로 Phase 1 의 실기기 QA 칸을 닫았다.
>
> **아직 안 본 것 두 가지** — 오픈 전에는 봐야 한다.
>
> | 남은 것 | 언제 |
> |---|---|
> | iPhone (Safari) 전체 | 최종 디자인 점검 (Phase 5) |
> | 햇빛 아래 가독성 | 최종 디자인 점검 (Phase 5) |
>
> 미룬 이유는 **디자인 디테일을 Phase 5 에 몰아서 다듬기로** 했기 때문이다.
> 화면이 한 번 더 바뀔 예정이라 지금 밖에 나가 확인해도 다시 해야 한다.
> **오픈 전에는 미룰 수 없다** — iOS 는 Safari 엔진만 쓰므로 안드로이드에서
> 멀쩡한 것이 거기서 깨질 수 있다.

브라우저의 모바일 모드는 **화면 크기만 줄인 것**이다. 실제 폰과 세 가지가
다르다 — iOS 는 Safari 엔진만 쓰고, 화면 밝기·색온도가 제각각이고,
손가락은 두께가 있다.

**먹빛 화면으로 바꾼 뒤라 전보다 중요해졌다.** 대비 16:1 은 계산값이고,
밝은 곳에서는 반사 때문에 훨씬 안 읽힌다.

주소: `namjosunhero.vercel.app` (검색에는 안 걸리지만 직접 치면 들어간다)

### G-1. iPhone (Safari) 과 Android (Chrome) **둘 다**

> 아래 체크박스는 **기기마다 다시** 채운다. 2026-09-13 에 갤럭시로 한 번
> 돌았고 `밖에서 한 번 본다` 를 뺀 나머지가 통과했다. 박스를 비워 둔 것은
> iPhone 이 남아 있기 때문이다.

- [ ] **밖에서 한 번 본다** — 햇빛 아래에서 목록 제목이 읽히는가.
      실내에서만 보면 절대 모른다
- [ ] 글 하나를 **끝까지 읽어본다** — 눈이 금방 피로해지지 않는가
- [ ] 제목·메뉴·버튼이 **명조**로 나오는가 (고딕이면 글꼴 로딩 실패)
- [ ] 그림이 뭉개지거나 늘어나 보이지 않는가

### G-2. 손가락으로 눌러보기

- [ ] 하단 탭 4개가 헛눌림 없이 눌리는가
- [ ] 댓글의 `답글` · `수정` · `삭제` 가 서로 잘못 눌리지 않는가
- [ ] 카테고리 칩을 가로로 넘길 때 걸리지 않는가
- [ ] 버튼 글자가 두 줄로 깨지지 않는가

### G-3. 입력

- [ ] 가입 폼에서 **키보드가 올라왔을 때 입력칸이 가려지지 않는가**
- [ ] 댓글 입력 후 화면이 엉뚱한 곳으로 튀지 않는가
- [ ] 약관 체크박스가 손가락으로 눌리는가 (라벨 전체가 눌려야 한다)

### G-4. 스크롤

- [ ] 스크롤할 때 상단 로고와 카테고리 줄이 **겹치거나 떨리지 않는가**
- [ ] iOS 에서 주소창이 숨었다 나타날 때 레이아웃이 튀지 않는가
- [ ] 가로 스크롤이 생기지 않는가 (본문이 화면 밖으로 나가면 안 된다)

### G-5. 기록

- [ ] 문제를 발견하면 **기기·OS 버전·화면 크기**를 함께 적는다.
      "안 보인다" 만으로는 재현할 수 없다
